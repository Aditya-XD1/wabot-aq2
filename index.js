...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:tbz.\n' //ganti nama lu! 
            + 'ORG: tbz.;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=62814622392081:+62 814622392081\n'  //ganti nomor lu! 
            + 'END:VCARD'
...
/******** OWNER NUMBER**********/
const ownerNumber = ["62814622392081@s.whatsapp.net"]   //ganti nomor lu! 
...
pushnem = client.contacts[num] != undefined ? client.contacts[num].notify = undefined ? PhoneNumber('+' + num.replace('@s.whatsapp.net', '')).getNumber('international') : client.contacts[num].notify || client.contacts[num].vname : PhoneNumber('+' + num.replace('@s.whatsapp.net', '')).getNumber('international')
			try {
				...
				.on('exit', () => {
			client.sendMessage(mdata.id, fs.readFileSync('hasil.jpg'), MessageType.image, {caption: `*Met Dateng🗿, Jan lupa intro dan baca rules bruh...* @${num.split('@')[0]}`, contextInfo: { mentionedJid: [num] }})
			})
...
pushnem = client.contacts[num] != undefined ? client.contacts[num].notify = undefined ? PhoneNumber('+' + num.replace('@s.whatsapp.net', '')).getNumber('international') : client.contacts[num].notify || client.contacts[num].vname : PhoneNumber('+' + num.replace('@s.whatsapp.net', '')).getNumber('international')
			try {
				...
				.on('error', () => reply('error'))
				.on('exit', () => {
			client.sendMessage(mdata.id, fs.readFileSync('hasil.jpg'), MessageType.image, {caption: `*Sayonara👋🏻*@${num.split('@')[0]}\n`, contextInfo: { mentionedJid: [num] }})
			})
		}
	} catch (e) {
		console.log(e)
...
client.on('CB:action,,call', async json => {
    ...
    console.log("call dari"+ callerId)
        client.sendMessage(callerId, "*Mohon Maaf Anda Menelpon Bot, Anda Akan Saya Block Thanks!*\n*_BY RIU_*", MessageType.text)
        await sleep(4000)
...
break
		case 'sewabot1':
                ...
                if (!isRegistered) return reply(ind.noregis())
                    ...
                    client.sendMessage('62814622392081@s.whatsapp.net', options, text, {quoted: mek})
		...
		case 'batal1':
                ...
                if (!isRegistered) return reply(ind.noregis())
                    ...
                    client.sendMessage('6283807588767@s.whatsapp.net', options, text, {quoted: mek})
                    reply('Anda Telah Membatalkan Pesanan!.')
                    break
                    case 'batal3':
				if (isBanned) return reply(ind.baned())
                if (!isRegistered) return reply(ind.noregis())
                    ...
                    client.sendMessage('6282126046864@s.whatsapp.net', options, text, {quoted: mek})
                    reply('Anda Telah Membatalkan Pesanan!.')
                    break
                    case 'batal4':
				if (isBanned) return reply(ind.baned())
                if (!isRegistered) return reply(ind.noregis())
                    ...
                    client.sendMessage('6283872842057@s.whatsapp.net', options, text, {quoted: mek})
                    reply('Anda Telah Membatalkan Pesanan!.')
                    break
		case 'sewabot2':
				if (isBanned) return reply(ind.baned())
...
ow2 = "6283807588767@s.whatsapp.net"
ow3 = "6282126046864@s.whatsapp.net"
ow4 = "6283872842057@s.whatsapp.net"
daftarnye = `
@----------DAFTAR SEWA----------@
...
pp_riu = await getBuffer(pp_user)
                    ...
                    client.sendMessage(from, pp_riu, image, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage":{"product":{"productImage":{"mimetype":"image/jpeg","height":736,"width":736,"jpegThumbnail":fs.readFileSync(`./me.jpg`)},"productId":"3937202479680283","title":"©dokidokinime","currencyCode":"IDR","priceAmount1000":"999999999000","productImageCount":1},"businessOwnerJid":"0@s.whatsapp.net"}}}, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: ini_txt })
                    break
		    case 'jawakelatin': 
				if (!isRegistered) return reply(ind.noregis())
...
─────────────────────`
		...
		case 'grouplist':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					client.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Ini adalah list group riu :\n\n\`\`\``
					no = 0
...
pp_riu = await getBuffer(pp_user)
                    client.sendMessage(from, pp_riu, image, {quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [ow, num]}, displayname: mem, caption: menusu })
					break
					case 'playstore':
		if (!isRegistered) return reply(ind.noregis())
...
case 'bneon':
		...
		case 'tagall2':
					...
					mentions(`┌───「RIU TAG」───`+ teks +'└───「 *_Pxc7_* 」────', members_id, true)
					break
		case 'clearall':
					if (!isOwner) return reply(ind.ownerb())
		...
		case 'group':
            ...
            case 'developer':
                  ...
                  client.sendMessage(from, 'Tuh nomer owner ku >_<, jangan spam atau ku block kamu',MessageType.text, { quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true}} )
					break    
           case 'kick':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
...
case 'upswimg':
		...
		reply(`*_「 Link Terdeteksi 」_*\nHei *${pushname}* Anda Mengirim Link Grup!, Anda Akan Segera Di Kick Dari Grup *${groupMetadata.subject}*`)
		setTimeout( () => {
	...
	if (body.startsWith(`${prefix}${command}`)) {
		num = `${sender.split("@")[0]}@s.whatsapp.net`
comd = `ʜᴇɪ @${num.split('@')[0]} !!!\nᴘᴇʀɪɴᴛᴀʜ/ᴄᴏᴍᴍᴀɴᴅ *${prefix}${command}*\nᴛɪᴅᴀᴋ ᴀᴅᴀ ᴅᴀʟᴀᴍ *${prefix}menu bot*\n`
client.sendMessage(from, comd, text,{quoted : mek, sendEphemeral: true, thumbnail: fs.readFileSync('./me.jpg', 'base64'), contextInfo: { forwardingScore: 1, isForwarded: true, mentionedJid: [num]}})
				  }
...
client.sendMessage(from, wa3, text,{quoted : mek, sendEphemeral: true, thumbnail: fs.readFileSync('./me.jpg', 'base64'), contextInfo: { forwardingScore: 1, isForwarded: true}})
                  }
       if (budy.includes(`bot`)) {
       	wa4 = `Ya?\nUntuk Menggunakan Bot Ketik ${prefix}menu/help`
client.sendMessage(from, wa4, text,{quoted : mek, sendEphemeral: true, thumbnail: fs.readFileSync('./me.jpg', 'base64'), contextInfo: { forwardingScore: 1, isForwarded: true}})
                  }
        if (budy.includes(`Woy`)) {
        	wa5 = `Ya?\nUntuk Menggunakan Bot Ketik ${prefix}menu/help`
client.sendMessage(from, wa5, text,{quoted : mek, sendEphemeral: true, thumbnail: fs.readFileSync('./me.jpg', 'base64'), contextInfo: { forwardingScore: 1, isForwarded: true}})
                  }
		if (budy.includes(`Hai`)) {
wa6 = `Hai Juga\nUntuk Menggunakan Bot Ketik ${prefix}menu/help`
client.sendMessage(from, wa6, text,{quoted : mek, sendEphemeral: true, thumbnail: fs.readFileSync('./me.jpg', 'base64'), contextInfo: { forwardingScore: 1, isForwarded: true}})
                  }
		if (budy.includes(`Hallo`)) {
			wa7 = `Hallo Juga\nUntuk Menggunakan Bot Ketik ${prefix}menu/help`
client.sendMessage(from, wa7, text,{quoted : mek, sendEphemeral: true, thumbnail: fs.readFileSync('./me.jpg', 'base64'), contextInfo: { forwardingScore: 1, isForwarded: true}})
                  }
		if (budy.includes(`Thanks`)) {
			wa8 = `Sama-sama ${pushname}`
client.sendMessage(from, wa8, text,{quoted : mek, sendEphemeral: true, thumbnail: fs.readFileSync('./me.jpg', 'base64'), contextInfo: { forwardingScore: 1, isForwarded: true}})
                  }
		if (budy.includes(`Makasih`)) {
			wa9 = `Sama-sama ${pushname}`
client.sendMessage(from, wa9, text,{quoted : mek, sendEphemeral: true, thumbnail: fs.readFileSync('./me.jpg', 'base64'), contextInfo: { forwardingScore: 1, isForwarded: true}})
                  }
